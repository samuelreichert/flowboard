...
The system SHALL provide reusable accessible presentation primitives for
panel-level and inline remote-data loading and recoverable error states while
keeping feature-specific copy and retry behavior explicit.
...
- **AND** the loading state exposes appropriate busy or status semantics to
...
- **WHEN** a background refresh, pagination request, or detail-content request
  fails while other resolved content remains usable
...
- **THEN** its feature-specific title, description, and action labels come from
  the resolved Flowboard localization catalog
