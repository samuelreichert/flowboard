...
| ----------------------------------------- | ------------------------------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| Local Prisma SQLite                       | `npm run dev`                                           | Local development principal when Supabase browser env is empty | Prisma structured tables read through `/api/board/bootstrap` and `/api/board/cards/:cardId`; legacy `/api/boards/default` and `/api/boards/:id` remain temporary history/save bridges | `PRISMA_PROVIDER=sqlite`; `DATABASE_URL=file:./data/flowboard.local.db`                                                    | Leave `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`, and `VITE_FLOWBOARD_API_URL` empty                                                                        | Optional `PORT`; defaults to Prisma SQLite when `PRISMA_PROVIDER` is unset                                            | This is the default local mode. Use this for normal development.      |
| Local production build with Prisma SQLite | `npm run build:local`, then `npm start`                 | Local development principal only with explicit server flag     | Prisma structured tables read through `/api/board/bootstrap` and `/api/board/cards/:cardId`; legacy `/api/boards/default` and `/api/boards/:id` remain temporary history/save bridges | `PRISMA_PROVIDER=sqlite`; `DATABASE_URL=file:./data/flowboard.local.db`                                                    | Leave Supabase browser env empty for no-auth local mode                                                                                                               | `FLOWBOARD_LOCAL_DEV_AUTH=true`; optional `PORT`                                                                      | Use this to test a production bundle against the local Node API.      |
| Local authenticated Prisma SQLite         | `npm run dev` with Supabase env configured              | Supabase Auth                                                  | Authenticated reads use `/api/board/bootstrap` and `/api/board/cards/:cardId`; active card saves use `/api/board/cards` resource mutations; non-card saves still bridge through `/api/boards/:id`     | `PRISMA_PROVIDER=sqlite`; `DATABASE_URL=file:./data/flowboard.local.db`                                                    | `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`; optional OAuth flags; optional `VITE_SUPABASE_AVATAR_BUCKET`                                                    | `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`; `PRISMA_PROVIDER=sqlite`; `DATABASE_URL`                                  | Best mode for local auth, profile, avatar, and structured board work. |
| Local authenticated Prisma Postgres       | `npm run dev` with Supabase and Postgres env configured | Supabase Auth                                                  | Authenticated reads use `/api/board/bootstrap` and `/api/board/cards/:cardId`; active card saves use `/api/board/cards` resource mutations; non-card saves still bridge through `/api/boards/:id`     | `PRISMA_PROVIDER=postgresql`; `DATABASE_URL=postgresql://...`; `DIRECT_URL=postgresql://...` for Prisma migration commands | `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`; optional OAuth flags; optional `VITE_SUPABASE_AVATAR_BUCKET`                                                    | `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`; `PRISMA_PROVIDER=postgresql`; `DATABASE_URL`                              | Use when reproducing production DB behavior locally.                  |
| Production authenticated app              | Deployed app plus Node API runtime                      | Supabase Auth                                                  | Authenticated reads use `/api/board/bootstrap` and `/api/board/cards/:cardId`; active card saves use `/api/board/cards` resource mutations; non-card saves still bridge through `/api/boards/:id` or `VITE_FLOWBOARD_API_URL` | Supabase Postgres via Prisma                                                                                               | `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`, OAuth flags, `VITE_SUPABASE_AVATAR_BUCKET`; set `VITE_FLOWBOARD_API_URL` only when the API is on another origin | `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, `PRISMA_PROVIDER=postgresql`, `DATABASE_URL`; `DIRECT_URL` for migrations | This is the intended durable production mode.                         |
...
- Without Supabase browser config, `npm run dev` uses the local no-auth Prisma
  path by calling the same bootstrap, card-detail, and temporary legacy bridge
